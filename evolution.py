...
variance=1
es=cma.CMAEvolutionStrategy(parameters,variance,inopts={'popsize':20})
j=0
whenToCopy=100
agent=AgentNetwork()
agent.cuda()
...
with tf.device('/GPU:0'):
    while True:
        ...
        for i in generatedParameters:
            ...
            env=Gymenv1player(agent=agent,num=1,maxsteps=10000,verbose=False,gameName=game)
            fitness.append(100-env.play())
        es.tell(generatedParameters,fitness)
        agent.loadparameters(es.result.xbest)
        env=Gymenv1player(agent=agent,num=1,maxsteps=100,verbose=False,gameName=game)
